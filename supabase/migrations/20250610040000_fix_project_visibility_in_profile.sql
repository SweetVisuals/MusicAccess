...
-- Recreate the function with the correct logic to calculate stats and fetch projects with tracks
...
BEGIN
  ...
  -- Fetch projects with tracks, including allow_download status
  SELECT json_agg(
    json_build_object(
      'id', proj.id,
      'title', proj.title,
      ...
      'allow_downloads', proj.allow_downloads,
      'tracks', (
        SELECT json_agg(
          json_build_object(
            'id', t.id,
  ...
  WHERE proj.user_id = p_user_id AND (proj.visibility = 'public' OR auth.uid() = p_user_id);
