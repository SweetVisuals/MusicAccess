DROP FUNCTION IF EXISTS search_profiles_and_projects(TEXT);
...
RETURNS TABLE(
    ...
    is_public BOOLEAN, -- Changed from is_popular to is_public
    price NUMERIC,
    gems INTEGER,
    tags TEXT[],
    location TEXT,
...
BEGIN
    ...
    SELECT
        ...
        p.updated_at,
        p.allow_downloads,
        p.is_public, -- Changed from p.is_popular to p.is_public
        p.price,
        p.gems,
        p.tags,
    ...
    SELECT
        ...
        NULL::BOOLEAN AS is_public, -- Changed from NULL::BOOLEAN AS is_popular to NULL::BOOLEAN AS is_public
