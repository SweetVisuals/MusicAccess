...
export interface FileWithMetadata extends File {
  ...
  preview?: string; // Add preview property for object URL
}

interface UploadDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onUpload: (publicUrl: string) => Promise<void>; // Change onUpload signature
  type?: 'avatar' | 'banner';
}
...
export const UploadDialog = ({ open, onOpenChange, onUpload, type }: UploadDialogProps) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      let publicUrl = ''; // Declare publicUrl outside the loop
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        // Get public URL
        const { data: { publicUrl: uploadedPublicUrl } } = supabase.storage // Use a different variable name here
          .from(folderName)
          .getPublicUrl(filePath);
          
        publicUrl = uploadedPublicUrl; // Assign to the outer publicUrl variable
        
        // Removed Supabase profile update from here
      }
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>
            {type === 'avatar' ? 'Upload Profile Image' : 'Upload Banner Image'}
          </DialogTitle>
        </DialogHeader>
              ...
              <div className="space-y-4">
                {files.map(file => (
                  <div key={file.id} className="relative">
                    <img
                      src={file.preview} // Use the stored preview URL
                      alt="Preview"
