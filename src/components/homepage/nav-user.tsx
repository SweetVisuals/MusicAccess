    ...
    <SidebarMenu>
      ...
      {authUser && !loadingStorage && ( // Only show if authUser exists and not loading storage
        <div className="px-3 py-2 border-b border-sidebar-border/30">
          ...
          <div className={`group-data-[collapsible=icon]:flex group-data-[collapsible=icon]:justify-center group-data-[collapsible=icon]:items-center group-data-[collapsible=icon]:py-1 ${state === 'expanded' ? 'hidden' : ''}`}>
            <span className="text-[0.6rem] font-medium text-sidebar-foreground/60 bg-sidebar-accent/30 px-1.5 py-0.5 rounded-full">
              {formatStorage(storageUsed).replace(/\s*([A-Z]+)$/, '')}/{formatStorage(storageLimit).replace(/\s*([A-Z]+)$/, '')}
            </span>
          </div>

          {/* Expanded view - full storage info */}
          <div className={`group-data-[collapsible=icon]:hidden ${state === 'collapsed' ? 'hidden' : ''}`}>
            ...
            <Progress
              ...
              className="h-2 bg-sidebar-accent/50 [&>div]:bg-sidebar-primary"
            />
          </div>
        </div>
      )}
      ...
      <SidebarMenuItem>
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <SidebarMenuButton
              size="lg"
              className="data-[state=open]:bg-sidebar-accent data-[state=open]:text-sidebar-accent-foreground group/user-button transition-all duration-200"
            >
              <Avatar className="h-9 w-9 rounded-full border-2 border-sidebar-border/50 group-hover/user-button:border-sidebar-primary/50 group-data-[state=open]/menu-button:border-sidebar-primary/50 transition-colors duration-200">
                <AvatarImage 
                  src={profile.profile_url || ''} 
                  alt={profile.username || ''}
                  className="group-hover/user-button:scale-110 group-data-[state=open]/menu-button:scale-110 transition-transform duration-200"
                />
                <AvatarFallback className="rounded-full bg-sidebar-accent text-sidebar-foreground font-semibold group-hover/user-button:bg-sidebar-primary/20 group-data-[state=open]/menu-button:bg-sidebar-primary/20 transition-colors duration-200">
                  {profile.username?.charAt(0)?.toUpperCase() || authUser.email?.charAt(0)?.toUpperCase() || 'U'}
                </AvatarFallback>
              </Avatar>
              <div className="grid flex-1 text-left text-sm leading-tight">
                <span className="truncate font-medium text-sidebar-foreground group-hover/user-button:text-sidebar-primary group-data-[state=open]/menu-button:text-sidebar-primary transition-colors duration-200">
                  {profile.username || authUser.email?.split('@')[0]}
                </span>
                <span className="truncate text-xs text-sidebar-foreground/60 group-hover/user-button:text-sidebar-primary/70 group-data-[state=open]/menu-button:text-sidebar-primary/70 transition-colors duration-200">
                  {profile.email || authUser.email}
                </span>
              </div>
              <MoreVerticalIcon className="ml-auto size-4 text-sidebar-foreground/60 group-hover/user-button:text-sidebar-primary group-data-[state=open]/menu-button:text-sidebar-primary transition-colors duration-200" />
            </SidebarMenuButton>
          </DropdownMenuTrigger>
          <DropdownMenuContent
